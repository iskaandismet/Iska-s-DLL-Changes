...
void CvTeam::SetAllowsOpenBordersToTeam(TeamTypes eIndex, bool bNewValue)
{
	...
	if(IsAllowsOpenBordersToTeam(eIndex) != bNewValue)
	{
		...
		if (pkScriptSystem) 
		{
			...
			bool bResult = false;
			LuaSupport::CallHook(pkScriptSystem, "SetHasOpenBorders", args.get(), bResult);
		}
	}
}
...
void CvTeam::SetHasAlliance(TeamTypes eIndex, bool bNewValue)
{
	...
	if(IsHasAlliance(eIndex) != bNewValue)
	{
		...
		GET_TEAM(eIndex).setPermanentWarPeace(GetID(), bNewValue);
		GET_TEAM(GetID()).setPermanentWarPeace(eIndex, bNewValue);
		GET_TEAM(eIndex).SetAllowsOpenBordersToTeam(GetID(), bNewValue);
		GET_TEAM(GetID()).SetAllowsOpenBordersToTeam(eIndex, bNewValue);
	}
}
...
void CvTeam::setVassal(TeamTypes eIndex, bool bNewValue, bool bVoluntary)
{
	...
	if (pkScriptSystem)
	{
		...
		args->Push(eIndex);
		args->Push(bNewValue);
		args->Push(bVoluntary);

		bool bResult;
		LuaSupport::CallHook(pkScriptSystem, "NewVassal", args.get(), bResult);
	}
}
